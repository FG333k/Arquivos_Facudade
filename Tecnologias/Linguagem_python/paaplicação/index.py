...
loginl.place(x=10, y=100) # Exibe a label na posição determinada 
logini = Entry(rframe, width=35) # Insere uma caixa de texto 
logini.place(x=50, y=100) # Exibe a caixa de texto na posição determinada

#Input senha
senhal = Label(rframe, text="Sanha", fg="white", bg="black") # insere uma label
senhal.place(x=7, y=140) # Exibe a label na posição determinada
senhai = Entry(rframe, width=35) # Insere uma caixa de texto
senhai.place(x=50, y=140) # Exibe a caixa de texto na posição determinada
...
def Login():
    ...
    if (login == '' or senha == ''):
        messagebox.showwarning(title="Atenção", message='Preencha todos os capos')
    else:
        try:
...
#Botão de login
loginbt = Button(rframe, text="Logar", width=8, command=Login) # Cria o botão para logar no sistema
loginbt.place(x=60, y=220) # Exibe na posição determinada
...
def cadastro():
    ...
    #Lado esquerdo
    #Titulo
    titulo.configure(text="CADASTROS")
    titulo.place(x=90)
    #Logo
    logol.place(x=-3000)
    ...
    nomel.place(x=5, y=70) # Exibe a label na posição determinada
    nomei = Entry(rframe, width=35) # Insere uma caixa de texto
    nomei.place(x=50, y=70) # Exibe a caixa de texto na posição determinada

    # Imput email
    emaill = Label(rframe, text="Email", bg="black", fg="white") # insere uma labe
    emaill.place(x=5, y=100) # Exibe a label na posição determinada
    emaili = Entry(rframe, width=35) # Insere uma caixa de texto
    emaili.place(x=50, y=100) # Exibe a caixa de texto na posição determinada

    
    #Caixa com as listas de cadastros
    listbox = Listbox(lframe, width=45, height=11) # Insere uma caixa de listas
    listbox.insert(END, "LISTA DE CADASTROS:")
    listbox.place(x=20, y=70) # Exibe na posição determinada 
    ...
    listbt = Button(rframe, text="Listar", width=8, command=listar)# Cria o botão para listar usuários/cadastros na list box
    listbt.place(x=40, y=220)# Exibe na posição determinada
    ...
    # Botão de inserir
    inserirbt = Button(rframe, text="Adicionar", width=8, command=inserir) # Cria o botão para inserir cadastros 
    inserirbt.place(x=112, y=220) # Exibe na posição determinada
    ...
    def voltar():
        ...
        #Lado esquerdo
        #Título
        titulo.configure(text="HELO-WORD")
        titulo.place(x=100)
        #Logo
    ...
    #Botão para voltar
    voltarbt = Button(rframe, text="Voltar", width=8, command=voltar) # Cria o botão para retornar a pagina tela inicial
    voltarbt.place(x=182, y=220) # Exibe na posição determinada

# Botaõ de cadastro
cadbt = Button(rframe, text="Cadastrar", width=8, command=cadastro) # Cria o botão para acessar o formulário de cadarstros
cadbt.place(x=180, y=220) # Exibe na posição determinada
